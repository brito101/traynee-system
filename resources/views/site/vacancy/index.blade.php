...
@section('content')
    <main>
                    ...
                    <div class="vacancies-image">
                        ...
                        @elseif ($vacancy->brand_twitter != null)
                            <img src="{{ url('storage/vacancies/' . $vacancy->brand_twitter) }}"
                                alt="{{ $vacancy->title }}" width="920" height="1040">
                        @else
                            <img src="{{ asset('/site/img/fotos/vacancy-0.webp') }}" alt="{{ $vacancy->title }}"
                                width="920" height="1040">
                        @endif
                    </div>
                    <div class="vacancies-content">
                        <h2 class="font-1-xl">{{ $vacancy->title }}</h2>
                        <p class="font-2-s {{ $loop->iteration % 2 == 0 ? 'color-0' : 'color-12' }}">Para os seguintes
                            cursos:
                        ...
                        <a class="button arrow" href="{{ route('vacancy', ['slug' => $vacancy->slug]) }}">Mais
                            Sobre</a>
                    </div>
                </div>
                @if ($loop->iteration % 2 == 0)
        ...
        @else
            <div class="vacancies container">
                <h2 class="font-1-xxl color-12">Estamos preparando excelentes vagas para você<span class="color-p1">
                        :)</span></h2>
            </div>
        @endif
