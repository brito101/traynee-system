...
@section('content')
    @if (auth()->user()->can('Editar Avaliações') &&
        ...
        @php
            ...
            foreach ($evaluations as $evaluation) {
                $list[] = [$evaluation->id, $evaluation->getTrainee(), $evaluation->vacancy->title, $evaluation->vacancy->company->alias_name, $evaluation->status, $evaluation->getCompability(), '<nobr>' . '<a class="btn btn-xs btn-default text-secondary mx-1 shadow" title="Visualizar" href="trainee/' . $evaluation->trainee . '" target="_blank"><i class="fa fa-lg fa-fw fa-eye"></i></a>' . $evaluation->link() . '<a class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar" href="evaluations/' . $evaluation->id . '/edit"><i class="fa fa-lg fa-fw fa-pen"></i></a>' . '<a class="btn btn-xs btn-default text-danger mx-1 shadow" title="Excluir" href="evaluations/destroy/' . $evaluation->id . '" onclick="return confirm(\'Confirma a exclusão desta alocação?\')"><i class="fa fa-lg fa-fw fa-trash"></i></a>'];
            }
    ...
    @else
        @php
            ...
            foreach ($evaluations as $evaluation) {
                $list[] = [$evaluation->id, $evaluation->getTrainee(), $evaluation->vacancy->title, $evaluation->vacancy->company->alias_name, $evaluation->status, $evaluation->getCompability(), '<nobr>' . '<a class="btn btn-xs btn-default text-primary mx-1 shadow" title="Visualizar" href="trainee/' . $evaluation->trainee . '" target="_blank"><i class="fa fa-lg fa-fw fa-eye"></i></a>' . $evaluation->link()];
            }
